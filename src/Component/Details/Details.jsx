...
export default function Details() {
    let {getCardData} = useContext(CartContext)
    let [api, setapi] = useState(null)
    let [loading, setloading] = useState(true)
    let [related, setrelated] = useState([])
    let {id , name } = useParams()
    let {addList} = useContext(ContextList)
  ...
  {loading == true ? <div  className="flex justify-center h-screen items-center" role="status">
...
</div> :  <div className='flex gap-5 flex-col md:flex-row items-center w-3/4 mx-auto py-10 justify-center '>
    ...
    <div className='w-full md:w-2/3'>
     ...
     <button onClick={()=> {getCardData(itemId)}} className=" w-3/4 translate-x-[15%] bg-[#5aa84f] text-slate-50 py-1 mt-2 rounded-md">Add</button>
     <i onClick={()=> addList(itemId)} className="fa-solid fa-heart translate-x-48 text-3xl"></i>
